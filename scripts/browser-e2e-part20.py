...
SHELL_V22 = PUBLIC / 'merlin-shell-v22.js'
...
def stripped_html() -> str:
    ...
    html = re.sub(r'<script(?:\s+type="module")?\s+src="[^"]+"(?:\s+defer)?\s*></script>', '', html)
...
async def load_page(browser, name: str, width: int, height: int):
    ...
    await page.add_script_tag(content=SHELL_V22.read_text(encoding='utf-8'))
