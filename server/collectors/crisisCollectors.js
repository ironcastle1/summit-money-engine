...
async function collectUSGS() {
  ...
  try {
    ...
    const events = (data.features || []).map(f => {
      ...
      return { id: stableId(`usgs:${p.code}:${p.time}`), kind: "crisis", subtype: "earthquake", severity: Number(p.mag) >= 5 ? 4 : 2, title: p.title || "Earthquake", summary: `${p.mag || "N/A"} magnitude earthquake`, place: p.place || "", country: "", lat: Number(c[1]), lng: Number(c[0]), source: "USGS", sourceSystem: "USGS", url: p.url || "", publishedAt: p.time ? new Date(p.time).toISOString() : null, confidence: "official-feed" };
...
async function collectNWS() {
  ...
  try {
    ...
    const events = (data.features || []).map(f => {
      const p = f.properties || {}, c = centroidFromGeometry(f.geometry); if (!c) return null;
      return { id: stableId(`nws:${p.id}:${p.sent}`), kind: "crisis", subtype: "weather-alert", severity: p.severity === "Extreme" || p.severity === "Severe" ? 4 : 2, title: `${p.event || "Weather alert"} - ${p.areaDesc || ""}`, summary: stripHtml(p.headline || p.description || "Weather alert").slice(0, 500), place: p.areaDesc || "United States", country: "United States", lat: c.lat, lng: c.lng, source: "US National Weather Service", sourceSystem: "NWS", url: p.uri || "", publishedAt: p.sent || p.effective, confidence: "official-alert" };
    }).filter(Boolean);
