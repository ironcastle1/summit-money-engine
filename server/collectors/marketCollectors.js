...
async function stooqQuote(symbol, id, name, type = "commodity") {
  const url = `https://stooq.com/q/l/?s=${encodeURIComponent(symbol)}&f=sd2t2ohlcv&h&e=csv`;
  try {
    const csv = await fetchText(url, { timeout: 9000 });
    ...
    return { id, type, symbol, name, price: close, changePct, source: "Stooq fallback" };
  } catch {
    return { id, type, symbol, name, price: null, changePct: null, source: "market fallback failed" };
  }
}

async function yahooChart(symbol, id, name, type = "commodity") {
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=1d&interval=5m`;
  try { const data = await fetchJson(url, { timeout: 9000 }); const result = data.chart && data.chart.result && data.chart.result[0]; const meta = result && result.meta || {}; const quote = result && result.indicators && result.indicators.quote && result.indicators.quote[0] || {}; const closes = (quote.close || []).filter(x => Number.isFinite(Number(x))); const price = Number(meta.regularMarketPrice) || closes[closes.length - 1]; const first = closes[0] || Number(meta.previousClose); const changePct = price && first ? ((price - first) / first) * 100 : null; if (!Number.isFinite(Number(price))) throw new Error("Yahoo no price"); return { id, type, symbol, name, price: Number(price), changePct, source: "Yahoo Finance" }; } catch { const stooqMap = { "GC=F":"gc.f", "SI=F":"si.f", "HG=F":"hg.f", "CL=F":"cl.f", "BZ=F":"brn.f", "NG=F":"ng.f", "ZW=F":"zw.f", "ZC=F":"zc.f", "ZS=F":"zs.f" }; return stooqQuote(stooqMap[symbol] || symbol.toLowerCase(), id, name, type); }
}
