...
function fallbackPlaceFromCoords(lat, lng) {
  ...
  if (nearest && nearest.distanceKm <= 45) {
...
async function nearbyInfrastructure(lat, lng, km = 4) {
  ...
  const query = `
    [out:json][timeout:22];
    (
      nwr["amenity"~"hospital|clinic|doctors|dentist|pharmacy|police|fire_station|embassy|fuel|ferry_terminal|bank|bureau_de_change"](${bbox.south},${bbox.west},${bbox.north},${bbox.east});
