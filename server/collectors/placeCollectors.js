...
const SEARCH_ALIASES = { damscus: 'damascus', damas: 'damascus', kyiv: 'Kyiv, Ukraine', kiev: 'Kyiv, Ukraine', 'old city damascus': 'Old City, Damascus', 'telaviv': 'Tel Aviv' };
...
async function reverseGeocode(lat, lng) {
  ...
  try {
    ...
    const fallback = fallbackPlaceFromCoords(lat, lng);
    if (fallback) return cache.set(key, fallback, TTL.reverse);
...
async function searchPlaces(q) {
  ...
  try {
    const rows = await fetchJson(url, { timeout: 4500 });
...
async function nearbyInfrastructure(lat, lng, km = 4) {
  ...
  const query = `
    [out:json][timeout:4];
    (
      nwr["amenity"~"hospital|clinic|doctors|dentist|pharmacy|police|fire_station|embassy|fuel|ferry_terminal|bank|bureau_de_change|drinking_water|shelter|charging_station|bus_station"](${bbox.south},${bbox.west},${bbox.north},${bbox.east});
      ...
      nwr["shop"~"supermarket|convenience|general|chemist|medical_supply|water|outdoor|hardware"](${bbox.south},${bbox.west},${bbox.north},${bbox.east});
  ...
  try {
    ...
    for (const endpoint of endpoints) {
      try {
        data = await fetchJson(`${endpoint}?data=${encodeURIComponent(query)}`, { timeout: 4200 });
    ...
    if (items.length < 8) {
      items.push(...estimatedUrbanInfrastructure(lat, lng, safeKm, items.length));
      items.sort((a, b) => {
        const p = infrastructurePriority(a.type) - infrastructurePriority(b.type);
        return p || a.distanceKm - b.distanceKm;
      });
    ...
    const result = { items, counts, nearestByType, source: "OpenStreetMap/Overpass + estimated urban access fallback", radiusKm: safeKm, radiusMiles: safeKm * 0.621371 };
...
function estimatedUrbanInfrastructure(lat, lng, km, existingCount = 0) {
  if (existingCount >= 8 || km < 1) return [];
  const types = [
    ["hospital", "Estimated hospital/clinic access area", 0.20, 0.08],
    ["pharmacy", "Estimated pharmacy access area", -0.12, 0.11],
    ["police_station", "Estimated police/emergency access area", 0.10, -0.13],
    ["fuel_station", "Estimated fuel/service access area", -0.18, -0.07],
    ["main_road", "Estimated main-road access", 0.00, 0.18],
    ["rail_station", "Estimated rail/transit access area", 0.15, 0.00],
    ["communications", "Estimated communications coverage area", -0.09, 0.20],
    ["food_supply", "Estimated food/supply access area", 0.09, -0.19]
  ];
  const maxOffset = Math.min(km * 0.22, 1.7);
  return types.map(([type, name, dy, dx], idx) => {
    const dLat = (dy * maxOffset) / 111;
    const dLng = (dx * maxOffset) / (111 * Math.max(0.25, Math.cos(Number(lat) * Math.PI / 180)));
    const itemLat = Number(lat) + dLat;
    const itemLng = Number(lng) + dLng;
    const distanceKm = haversineKm(Number(lat), Number(lng), itemLat, itemLng) || Math.min(km, 1);
    return {
      id: `estimated:${type}:${Number(lat).toFixed(3)}:${Number(lng).toFixed(3)}:${idx}`,
      name,
      lat: itemLat,
      lng: itemLng,
      type,
      ...
      estimated: true,
      tags: { estimated: "true" },
      source: "Estimated urban access fallback when OSM returns sparse local infrastructure"
    };
  });
}
...
async function wikiPlace(name) {
  ...
  const badTitle = value => /\b(road|highway|route|list of|timeline of|history of|battle of|treaty|election|football|album|film|song|1920|1930|1940|1950|century)\b/i.test(String(value || ""));
  const isRoadish = value => /^(road|street|lane|avenue|avenida|d\s*\d+|a\s*\d+|m\s*\d+|b\s*\d+|route\s*\d+)/i.test(value || "");

  function candidatesFor(q) {
    const parts = q.split(",").map(x => clean(x)).filter(Boolean);
    const out = [];
    const cityish = parts.find(x => x && !isRoadish(x) && !badTitle(x));
    const country = parts[parts.length - 1];
    if (cityish && country && cityish.toLowerCase() !== country.toLowerCase()) out.push(`${cityish}, ${country}`);
    if (cityish) out.push(cityish);
    for (const part of parts) if (part && !isRoadish(part) && !badTitle(part)) out.push(part);
    if (!badTitle(q)) out.push(q);
    return [...new Set(out.filter(Boolean))].slice(0, 8);
  }

  function scoreResult(title, snippet, candidate) {
    let score = 0;
    ...
    if (t === first) score += 80;
    if (t.includes(first) || first.includes(t)) score += 45;
    if (/city|town|village|municipality|district|neighbourhood|suburb|governorate|province|county|capital/i.test(`${title} ${snippet}`)) score += 25;
    if (badTitle(title)) score -= 90;
    if (/road|route|highway|battle|episode|film|song|album|football|election/i.test(`${title} ${snippet}`)) score -= 35;
    return score;
  }

  async function summaryFor(title) {
    const summary = await fetchJson(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(title)}?redirect=true`, { timeout: 4500 });
    let thumb = summary.thumbnail && summary.thumbnail.source || null;
    if (!thumb) {
      try {
        const images = await fetchJson(`https://en.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(summary.title || title)}&prop=pageimages&pithumbsize=700&format=json&origin=*`, { timeout: 4500 });
  ...
  try {
    for (const candidate of candidatesFor(query)) {
      const search = await fetchJson(`https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(candidate)}&format=json&origin=*&srlimit=5`, { timeout: 4500 });
      const rows = search.query && search.query.search || [];
      const ranked = rows
        .filter(row => row && row.title)
        .map(row => ({ row, score: scoreResult(row.title, row.snippet, candidate) }))
        .filter(x => x.score > 0)
        .sort((a, b) => b.score - a.score);
      for (const { row } of ranked) {
        const result = await summaryFor(row.title);
        if (result && !badTitle(result.title) && (result.thumbnail || result.extract)) {
          markSuccess("Wikipedia", result.thumbnail ? 1 : 0, "Loaded place summary/image with anti-random-page ranking");
          return cache.set(key, result, TTL.wiki);
        }
      }
    }
    markSuccess("Wikipedia", 0, "No place image match after ranked search");
    return cache.set(key, { found: false }, TTL.wiki);
  } catch (err) {
    markFailure("Wikipedia", err);
    return cache.set(key, { found: false, error: err.message }, 20 * 60 * 1000);
  }
}
