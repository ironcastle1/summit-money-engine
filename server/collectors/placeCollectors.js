...
function classifyInfrastructure(tags = {}) {
  ...
  if (["hospital"].includes(amenity) || emergency === "hospital") return "hospital";
...
async function searchPlaces(q) {
  ...
  try {
    ...
    return cache.set(key, places, TTL.search);
  } catch (err) {
...
async function nearbyInfrastructure(lat, lng, km = 4) {
  ...
  const query = `
    [out:json][timeout:22];
    (
      nwr["amenity"~"hospital|clinic|doctors|pharmacy|police|fire_station|embassy|fuel|ferry_terminal|bank|bureau_de_change"](${bbox.south},${bbox.west},${bbox.north},${bbox.east});
  ...
  try {
    const data = await fetchJson(url, { timeout: 24000 });
...
async function wikiPlace(name) {
  ...
  try {
    const search = await fetchJson(`https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(query)}&format=json&origin=*&srlimit=1`, { timeout: 10000 });
    const first = search.query && search.query.search && search.query.search[0];
    if (!first) {
      markSuccess("Wikipedia", 0, "No image match");
    ...
    const result = {
      ...
      url: summary.content_urls && summary.content_urls.desktop && summary.content_urls.desktop.page || `https://en.wikipedia.org/wiki/${encodeURIComponent(first.title.replace(/ /g, "_"))}`,
      source: "Wikipedia"
    };
