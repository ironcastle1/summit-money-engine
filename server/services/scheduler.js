...
async function refreshNow(){
  ...
  running = true;
  try{
    const [markets, klines, polymarket, events] = await Promise.all([fetchMarkets(), fetchCharts(), fetchPolymarket(), fetchEvents()]);
    ...
    setState({ markets, klines, polymarket, events, rapid, signals });
