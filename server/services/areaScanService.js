...
function estimateAreaMetrics({ localCrime, indicators, infrastructure, insideEvents, nearEvents, weather, advisory }) {
  ...
  const rawSafety = 72 - securityRisk * 0.38 + infraAccess * 0.12;
  // Conservative public-source estimate: avoid fake 99/1 extremes unless direct life-safety signals exist.
  const capHigh = (war || terror || advisoryRisk >= 65) ? 70 : 82;
  const capLow = (war || terror || advisoryRisk >= 75) ? 15 : 35;
  const safetyEstimate = Math.max(capLow, Math.min(capHigh, Math.round(rawSafety / 5) * 5));
...
async function runAreaScan({ query, radiusMiles, filter }) {
  ...
  const places = await withTimeout(searchPlaces(q), 4200, []);
  ...
  const place = await withTimeout(reverseGeocode(lat, lng), 4200, { displayName: target.displayName || target.name || q, name: target.name || q, country: target.country || (target.raw && target.raw.country) || "", countryCode: target.countryCode || (target.raw && target.raw.country_code ? String(target.raw.country_code).toUpperCase() : null), city: target.name || q, raw: target.raw || {}, source: "search fallback" });
  ...
  const [weather, infrastructure, wiki, indicators, advisory, localCrime] = await Promise.all([
    ...
    withTimeout(nearbyInfrastructure(lat, lng, radiusKm), 5200, { items: [], counts: {}, nearestByType: {}, source: "Overpass timeout", radiusKm, radiusMiles: radiusMi, timeout: true }),
    withTimeout(wikiPlace([place.city || place.name || target.name || q, place.country].filter(Boolean).join(", ")), 4200, { found: false, source: "Wikipedia timeout" }),
