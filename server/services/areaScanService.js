...
function estimateAreaMetrics({ localCrime, indicators, infrastructure, insideEvents, nearEvents, weather, advisory }) {
  ...
  if (localCrime && localCrime.available && Number.isFinite(Number(localCrime.total))) {
    ...
    crimeRisk = Math.max(0, Math.min(70, Math.round(Math.sqrt(total) * 4.2)));
  } else if (indicators && indicators.homicide && Number.isFinite(Number(indicators.homicide.value))) {
    crimeRisk = Math.max(0, Math.min(90, Math.round(Number(indicators.homicide.value) * 6.5)));
  }
  ...
  const safetyEstimate = Math.max(1, Math.min(99, Math.round(100 - securityRisk * 0.55 + infraAccess * 0.16)));
  const unsafeEstimate = Math.max(1, Math.min(99, 100 - safetyEstimate));
...
async function runAreaScan({ query, radiusMiles, filter }) {
  ...
  const places = await withTimeout(searchPlaces(q), 7000, []);
  ...
  const place = await withTimeout(reverseGeocode(lat, lng), 5000, { displayName: target.displayName || target.name || q, name: target.name || q, country: target.country || "", countryCode: target.countryCode || null, city: target.name || q, raw: {}, source: "search fallback" });
  ...
  const [weather, infrastructure, wiki, indicators, advisory, localCrime] = await Promise.all([
    withTimeout(weatherAt(lat, lng), 5000, { current: null, source: "weather timeout" }),
    withTimeout(nearbyInfrastructure(lat, lng, radiusKm), 10000, { items: [], counts: {}, nearestByType: {}, source: "Overpass timeout", radiusKm, radiusMiles: radiusMi, timeout: true }),
    withTimeout(wikiPlace([place.city || place.name || target.name, place.country].filter(Boolean).join(", ")), 6000, { found: false, source: "Wikipedia timeout" }),
    country ? withTimeout(getCountryIndicators(country.iso2), 6000, {}) : Promise.resolve({}),
    country ? withTimeout(fcdoAdvice(country), 6000, { available: false, level: "not loaded" }) : Promise.resolve({ available: false }),
    country && country.iso2 === "GB" ? withTimeout(ukCrimeRadius(lat, lng, radiusKm), 7000, { available: false, source: "data.police.uk timeout" }) : Promise.resolve({ available: false, source: "No official local crime feed connected for this country" })
  ]);
