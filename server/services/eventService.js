...
async function fetchEvents(){ const xfeed=await fetchXFeed(); const all=[...baselineMonitors,...(await fetchGdelt()),...(await fetchReliefWeb()),...eventsFromX(xfeed)]; const seen=new Set(); const events=all.filter(e=>{ if(!e||seen.has(e.id)) return false; seen.add(e.id); return true; }).slice(0,800); return { events, xfeed }; }
