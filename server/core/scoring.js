...
function crimeScore(localCrime, homicide) {
  if (localCrime && localCrime.available && Number.isFinite(Number(localCrime.total))) {
    ...
    return { value: clamp(96 - Math.sqrt(total) * 3.8, 35, 96), label: total < 25 ? "lower local police count" : total < 120 ? "normal urban local police count" : "elevated local police count", dataLevel: "local", source: localCrime.source, raw: `${total} local records` };
  }
  ...
  return { value: 72, label: "estimated: no local crime feed", dataLevel: "estimated", source: "no local crime source, using context", raw: "Estimated from public risk context, not official local crime" };
}
function buildRisk({ place, country, events, countryEvents, indicators, localCrime, weather, advisory }) {
  ...
  const safetyValue = clamp((crime.value === null ? 72 : crime.value) - danger * 0.32, 3, 98);
