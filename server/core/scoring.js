...
function crimeScore(localCrime, homicide) {
  if (localCrime && localCrime.available && Number.isFinite(Number(localCrime.total))) {
    ...
    return { value: clamp(Math.round((92 - Math.sqrt(total) * 1.6) / 5) * 5, 45, 90), label: total < 25 ? "lower local police count" : total < 120 ? "normal urban local police count" : "elevated local police count", dataLevel: "local", source: localCrime.source, raw: `${total} local records` };
  }
  if (homicide && Number.isFinite(Number(homicide.value))) {
    const rate = Number(homicide.value);
    return { value: clamp(Math.round((95 - rate * 5.5) / 5) * 5, 15, 90), label: rate < 2 ? "lower national homicide" : rate < 8 ? "moderate national homicide" : "higher national homicide", dataLevel: "national", source: homicide.source, raw: `${rate.toFixed(1)} homicide rate per 100k` };
  }
  return { value: 65, label: "estimated: no local crime feed", dataLevel: "estimated", source: "no local crime source, using context", raw: "Estimated from public risk context, not official local crime" };
}
function buildRisk({ place, country, events, countryEvents, indicators, localCrime, weather, advisory }) {
  ...
  const danger = warHits * 24 + terrorHits * 20 + crisisHits * 8 + politicsHits * 4 + movementHits * 3 + countryWarHits * 4 + countryTerrorHits * 4 + countryCrisisHits * 2 + weatherRisk + governancePenalty + advisoryPenalty;
