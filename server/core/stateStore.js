...
const { collectUSGS, collectGDACS, collectEONET, collectNWS } = require("../collectors/crisisCollectors");
...
async function refreshState(force = false) {
  ...
  try {
    const [gdelt, relief, usgs, gdacs, eonet, nws, marketPack] = await Promise.all([collectGdelt(), collectReliefWeb(), collectUSGS(), collectGDACS(), collectEONET(), collectNWS(), collectMarkets()]);
    const events = dedupe([...gdelt, ...relief, ...usgs, ...gdacs, ...eonet, ...nws]).slice(0, 650);
    const markets = marketPack.markets || [];
    const movers = marketSignals(markets, events);
    const countryRisk = buildCountryRisk(events);
    const familyWatch = buildFamilyWatch(events);
    const liveBrief = buildLiveBrief(events, movers, familyWatch);
    STATE.state = { version: "summit-security-companion-v1", updatedAt: new Date().toISOString(), events, markets, rapid: movers.slice(0, 30), predictions: movers.slice(0, 24), countryRisk, familyWatch, liveBrief, fx: marketPack.fx, product: "travel-security-family-safety-business-resilience", sourcesSummary: "No fake local data: local only when the source is local; otherwise national/live/estimated/missing labels are shown." };
...
function buildCountryRisk(events) {
  return countryMeta.map(c => {
    ...
    const risk = clamp(war * 28 + terror * 22 + crisis * 12 + politics * 8 + movement * 6 + (c.watch ? 8 : 0), 0, 100) || 0;
...
function buildLiveBrief(events, movers, familyWatch) {
  const buckets = ["war", "terror", "crisis", "politics", "movement", "money"].map(kind => ({ kind, items: events.filter(e => e.kind === kind).slice(0, 5) }));
  return { headline: "Travel security, family watch, movement disruption and markets", generatedAt: new Date().toISOString(), keyAlerts: events.filter(e => ["war", "terror", "crisis", "movement"].includes(e.kind)).slice(0, 10), buckets, topFamilyWatch: familyWatch.slice(0, 8), topMarketMovers: movers.slice(0, 8) };
}
