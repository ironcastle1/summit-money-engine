...
const ACTIVE_WAR_OVERLAY = new Set(["Ukraine","Russia","Syria","Sudan","Yemen","Somalia","Mali","Burkina Faso","Niger","Myanmar","Palestine","Israel","Lebanon","Iraq","Afghanistan"]);
const CONFLICT_ZONES = [
  { id: "ukraine-east-south", name: "Ukraine east/south active conflict belt", country: "Ukraine", type: "estimated conflict zone", source: "public conflict reporting estimate", polygon: [[51.4,34.6],[51.0,37.8],[49.7,39.2],[48.4,39.1],[47.1,37.7],[46.0,35.1],[46.3,32.1],[47.2,31.6],[48.4,32.8],[49.2,34.0],[50.4,34.5]] },
  { id: "russia-border-war-context", name: "Russia border war context zone", country: "Russia", type: "estimated conflict-adjacent zone", source: "public conflict reporting estimate", polygon: [[51.6,34.0],[51.6,40.0],[49.7,40.2],[49.5,37.8],[50.3,35.1]] },
  { id: "gaza", name: "Gaza active conflict zone", country: "Palestine", type: "estimated conflict zone", source: "public conflict reporting estimate", polygon: [[31.60,34.25],[31.60,34.58],[31.20,34.58],[31.20,34.25]] },
  { id: "israel-lebanon-border", name: "Israel/Lebanon border conflict zone", country: "Lebanon", type: "estimated conflict zone", source: "public conflict reporting estimate", polygon: [[33.35,35.18],[33.35,35.95],[32.95,35.95],[32.95,35.18]] },
  { id: "syria-northwest", name: "Northwest Syria conflict zone", country: "Syria", type: "estimated conflict zone", source: "public conflict reporting estimate", polygon: [[36.4,35.7],[36.4,37.7],[35.1,37.7],[35.1,35.7]] },
  { id: "syria-east", name: "Eastern Syria security zone", country: "Syria", type: "estimated conflict/security zone", source: "public security reporting estimate", polygon: [[35.7,39.0],[35.7,41.9],[33.8,41.9],[33.8,39.0]] },
  { id: "sudan-khartoum", name: "Khartoum conflict zone", country: "Sudan", type: "estimated conflict zone", source: "public conflict reporting estimate", polygon: [[16.2,31.8],[16.2,33.2],[14.9,33.2],[14.9,31.8]] },
  { id: "sudan-darfur", name: "Darfur conflict zone", country: "Sudan", type: "estimated conflict zone", source: "public conflict reporting estimate", polygon: [[15.9,21.0],[15.9,26.8],[10.0,26.8],[10.0,21.0]] },
  { id: "yemen-west", name: "Yemen western conflict/security zone", country: "Yemen", type: "estimated conflict/security zone", source: "public conflict reporting estimate", polygon: [[17.4,42.6],[17.4,45.5],[13.0,45.5],[13.0,42.6]] },
  { id: "somalia-south", name: "South/Central Somalia active security zone", country: "Somalia", type: "estimated security zone", source: "public security reporting estimate", polygon: [[4.5,42.5],[4.5,47.5],[-1.5,47.5],[-1.5,42.5]] },
  { id: "sahel-belt", name: "Central Sahel insurgency risk belt", country: "Mali", type: "estimated security zone", source: "public security reporting estimate", polygon: [[17.0,-4.8],[16.4,2.5],[13.0,5.0],[10.8,1.0],[11.0,-5.0],[14.0,-7.0]] }
];
...
async function refreshState(force = false) {
  ...
  try {
    ...
    STATE.state = { version: "summit-security-companion-v4-5-security-ops", updatedAt: new Date().toISOString(), events, markets, rapid: movers.slice(0, 30), predictions: movers.slice(0, 24), countryRisk, liveBrief, fx: marketPack.fx, product: "travel-security-business-resilience", sourcesSummary: "No fake local data: local only when the source is local; otherwise national/live/estimated/missing labels are shown." };
    STATE.mapData = { nodes: events.slice(0, 250), cityNodes: citySeeds.map(([name, country, lat, lng]) => ({ id: stableId(`${name}:${country}`), name, country, lat, lng, kind: "city", source: "city-seed" })), countryRisk, conflictZones: CONFLICT_ZONES };
...
function buildCountryRisk(events) {
  return countryMeta.map(c => {
    ...
    const crisis = countryEvents.filter(e => e.kind === "crisis" && !(e.subtype === "earthquake" && Number(e.magnitude || 0) < 5)).length;
    ...
    const risk = clamp(strictWar * 28 + terror * 22 + crisis * 12 + politics * 8 + movement * 6, 0, 100) || 0;
    return { country: c.name, iso2: c.iso2, region: c.region, centre: c.centre, watch: !!c.watch, familyPriority: !!c.familyPriority, overlayWar, war, terror, crisis, politics, movement, risk, colour: overlayWar ? "#ff174f" : risk >= 25 ? "#ff8c00" : "#00a66a" };
  });
}
