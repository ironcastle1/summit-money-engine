...
const ACTIVE_WAR_OVERLAY = new Set(["Ukraine","Syria","Sudan","Yemen","Somalia","Mali","Burkina Faso","Niger","Myanmar","Palestine","Israel","Lebanon","Russia"]);
...
function buildCountryRisk(events) {
  return countryMeta.map(c => {
    ...
    const overlayWar = strictWar > 0 && ACTIVE_WAR_OVERLAY.has(c.name);
...
function buildLiveBrief(events, movers) {
  const useful = (events || [])
    .filter(e => ["war", "terror", "movement", "politics", "crisis", "money"].includes(e.kind))
    .map(e => ({ ...e, importance: eventImportance(e) }))
    .filter(e => e.importance >= 35)
    .filter(e => !(e.subtype === "earthquake" && Number(e.magnitude || 0) < 5))
    .sort((a, b) => b.importance - a.importance);
  const buckets = ["war", "terror", "movement", "politics", "crisis", "money"].map(kind => ({
    kind,
