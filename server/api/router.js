...
router.get("/place/intel", async (req, res) => {
  ...
  const place = await withTimeout(reverseGeocode(lat, lng), 5000, { displayName: "Selected mapped place", name: "Selected place", country: "", countryCode: null, raw: {}, source: "reverse geocode timeout" });
  ...
  const [weather, infra, wiki, indicators, advisory, localCrime] = await Promise.all([
    ...
    withTimeout(nearbyInfrastructure(lat, lng, 5), 9000, { items: [], counts: {}, nearestByType: {}, source: "Overpass timeout" }),
    withTimeout(wikiPlace([place.city || place.name, place.country].filter(Boolean).join(", ")), 6000, { found: false, source: "Wikipedia timeout" }),
    country ? withTimeout(getCountryIndicators(country.iso2), 6000, {}) : Promise.resolve({}),
    country ? withTimeout(fcdoAdvice(country), 6000, { available: false }) : Promise.resolve({ available: false }),
    country && country.iso2 === "GB" ? withTimeout(ukCrimeRadius(lat, lng, 8.04672), 7000, { available: false, source: "data.police.uk timeout" }) : Promise.resolve({ available: false, source: "No official local crime feed connected for this country" })
  ]);
