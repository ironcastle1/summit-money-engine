...
router.get("/place/intel", async (req, res) => {
  ...
  const place = await withTimeout(reverseGeocode(lat, lng), 2500, { displayName: "Selected mapped place", name: "Selected place", country: "", countryCode: null, raw: {}, source: "reverse geocode timeout" });
  ...
  const [weather, infra, wiki, indicators, advisory, localCrime] = await Promise.all([
    ...
    withTimeout(nearbyInfrastructure(lat, lng, 5), 5500, { items: [], counts: {}, nearestByType: {}, source: "Overpass timeout" }),
