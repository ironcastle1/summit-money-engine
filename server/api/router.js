...
const v6Routes = require("./v6Routes");
const v7Routes = require("./v7Routes");
...
router.get("/infrastructure/radius", async (req, res) => {
  const lat = Number(req.query.lat), lng = Number(req.query.lng), km = Number(req.query.km || req.query.radiusKm || 2);
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) return res.status(400).json({ error: "lat/lng required" });
  res.json(await withTimeout(nearbyInfrastructure(lat, lng, Math.max(0.3, Math.min(6, km))), 6500, { items: [], counts: {}, nearestByType: {}, source: "Overpass timeout" }));
});
...
router.get("/place/intel", async (req, res) => {
  ...
  const place = await withTimeout(reverseGeocode(lat, lng), 4200, { displayName: "Selected mapped place", name: "Selected place", country: "", countryCode: null, raw: {}, source: "reverse geocode timeout" });
  const country = byIso(place.countryCode) || byName(place.country);
  if (!country && !place.countryCode && !place.country) {
    return res.json({ isUnresolved: true, oceanName: place.displayName || "No land/place resolved", place, country: null, dataRules: ["This click did not resolve to a usable country/place, so land safety and local crime scores are not shown."] });
  }
  const [weather, infra, wiki, indicators, advisory, localCrime] = await Promise.all([
    ...
    withTimeout(nearbyInfrastructure(lat, lng, 5), 5200, { items: [], counts: {}, nearestByType: {}, source: "Overpass timeout" }),
    withTimeout(wikiPlace([place.city || place.name, place.country].filter(Boolean).join(", ")), 3000, { found: false, source: "Wikipedia timeout" }),
    country ? withTimeout(getCountryIndicators(country.iso2), 3000, {}) : Promise.resolve({}),
    country ? withTimeout(fcdoAdvice(country), 3000, { available: false }) : Promise.resolve({ available: false }),
    country && country.iso2 === "GB" ? withTimeout(ukCrimeRadius(lat, lng, 8.04672), 4500, { available: false, source: "data.police.uk timeout" }) : Promise.resolve({ available: false, source: "No official local crime feed connected for this country" })
  ]);
