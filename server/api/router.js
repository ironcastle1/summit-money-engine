...
router.get("/place/intel", async (req, res) => {
  ...
  const nearbyEvents = (state.events || []).map(e => ({ ...e, distanceKm: haversineKm(lat, lng, e.lat, e.lng) })).filter(e => Number.isFinite(e.distanceKm) && e.distanceKm <= 700).sort((a, b) => a.distanceKm - b.distanceKm).slice(0, 20);
  const risk = buildRisk({ place, country, events: state.events, indicators, localCrime, weather, advisory });
  res.json({ place, country, risk, weather, infrastructure: infra, wiki, indicators, advisory, localCrime, nearbyEvents, dataRules: ["Local crime is only local where an official local feed exists.", "National indicators are not town-level.", "Live events are event-feed signals, not verified police records.", "Missing sources show missing instead of fake values."] });
});
router.get("/family-watch", async (req, res) => { const s = await refreshState(false); res.json({ familyWatch: s.familyWatch }); });
