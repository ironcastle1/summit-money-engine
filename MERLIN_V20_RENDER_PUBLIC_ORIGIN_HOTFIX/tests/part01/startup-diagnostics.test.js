...
test('Render hostname supplies the production public origin when PUBLIC_ORIGIN is absent', () => {
  const config = loadConfig({
    NODE_ENV: 'production',
    SESSION_SECRET: 'x'.repeat(48),
    RENDER_EXTERNAL_HOSTNAME: 'summit-money-engine-1.onrender.com'
  });
  assert.equal(config.accounts.publicOrigin, 'https://summit-money-engine-1.onrender.com');
  const diagnostics = buildStartupDiagnostics(config);
  assert.equal(diagnostics.blockers.some(entry => entry.code === 'PUBLIC_ORIGIN_PLACEHOLDER'), false);
});

test('Render hostname replaces the example.com placeholder', () => {
  const config = loadConfig({
    NODE_ENV: 'production',
    SESSION_SECRET: 'x'.repeat(48),
