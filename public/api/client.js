...
export function createApiClient(options = {}) {
  ...
  async function get(path, params = {}, requestOptions = {}) {
    ...
    try {
      const response = await fetch(url, { headers: { accept: 'application/json', 'x-client-version': '16.9.0-part-9' }, credentials: 'same-origin', signal: requestOptions.signal || controller.signal, cache: 'no-store' });
  ...
  async function post(path, body, requestOptions = {}) {
    ...
    try {
      const response = await fetch(url, {
        method: 'POST',
        headers: { accept: 'application/json', 'content-type': 'application/json', 'x-client-version': '16.9.0-part-9', ...(csrfToken ? { 'x-csrf-token': csrfToken } : {}) },
