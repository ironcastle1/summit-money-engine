...
import { ensureMapLibre } from './map/maplibre-loader.js';
import { FallbackWorldMap } from './map/fallback-world-map.js';

const VERSION = '17.1.0';
const api = createApiClient({ timeoutMs: 9_000 });
...
const state = {
  ...
  mapReady: false,
  mapStyle: 'liberty',
  layers: { alerts: true, news: true, earthquakes: true, disasters: true, conflict: true, routes: false, ports: false, countryRisk: false, heat: true },
  windowDays: 7,
  radiusKm: 250,
  events: [],
...
async function loadPreloads() {
  const files = await Promise.allSettled([
    fetchJson('/data/fallback-events.json'),
    fetchJson('/data/preload-news.json'),
    fetchJson('/data/preload-shipping.json'),
    fetchJson('/data/preload-shipping-catalog.json'),
    fetchJson('/data/preload-markets.json'),
    fetchJson('/data/preload-intelligence.json'),
    fetchJson('/data/preload-intelligence-catalog.json')
  ]);
  const value = index => files[index].status === 'fulfilled' ? files[index].value : {};
  state.events = value(0).events || [];
  state.news = value(1);
  state.shipping = value(2);
  state.shippingCatalog = value(3);
  state.markets = value(4);
  state.intelligence = value(5);
  state.intelligenceCatalog = value(6);
...
function renderTicker() {
  const items = (state.markets.results || []).map(result => {
    const quote = result.quote || {};
    const change = finite(quote.change24h) ? Number(quote.change24h) * 100 : null;
    return `<span class="ticker-item"><b>${escapeHtml(result.asset?.symbol || quote.symbol || '—')}</b><span>${finite(quote.price) ? formatMarketPrice(quote.price) : '—'}</span><em class="${change > 0 ? 'up' : change < 0 ? 'down' : 'neutral'}">${finite(change) ? formatPercent(change, 2, true) : '—'}</em></span>`;
  });
  $('#market-ticker-track').innerHTML = items.join('') || '<span class="ticker-item"><span>MARKET SNAPSHOT LOADING</span></span>';
}
...
function applyLayerVisibility() {
  if (state.mapKind === 'fallback') {
    state.map?.setRoutesVisible?.(state.layers.routes);
    state.map?.setClustersVisible?.(false);
    return;
  }
  const groups = {
...
function updateMapData() {
  if (state.mapKind === 'fallback') {
    state.map.setGlobalEvents(state.events);
    state.map.setRoutes(routesGeoJson());
    state.map.setRoutesVisible(state.layers.routes);
    return;
  }
...
async function initializeMap() {
  const result = await ensureMapLibre({ timeoutMs: 1_800 });
  if (!result.available || !window.maplibregl?.Map) {
    initializeFallbackMap();
    return;
  }
  const styles = mapStyleCandidates(state.mapStyle);
  let styleIndex = 0;
  const map = new window.maplibregl.Map({
    container: 'world-map', style: styles[0], center: [3, 24], zoom: 2.15, minZoom: 1.1, maxZoom: 16,
    pitch: 0, bearing: 0, dragPan: true, scrollZoom: true, doubleClickZoom: true, keyboard: true,
    touchZoomRotate: true, renderWorldCopies: true, attributionControl: true, fadeDuration: 100
  });
  state.map = map; state.mapKind = 'maplibre';
  ...
  let loaded = false;
  const fallbackTimer = setTimeout(() => { if (!loaded) { try { map.setStyle(rasterStyle()); } catch {} } }, 7_000);
  map.on('style.load', () => {
    loaded = true; clearTimeout(fallbackTimer); state.mapReady = true; $('#world-map').classList.add('map-ready');
    addMapLayers(); updateMapData();
  });
  map.on('error', event => {
    const message = String(event?.error?.message || '');
    if (loaded || !/style|sprite|glyph|source/i.test(message)) return;
    styleIndex += 1;
    if (styleIndex < styles.length) { try { map.setStyle(styles[styleIndex]); } catch {} }
    else { try { map.setStyle(rasterStyle()); } catch {} }
  });
  map.on('moveend', () => renderMapStatus());
  map.on('zoomend', () => renderMapStatus());
  setTimeout(() => map.resize(), 50);
  setTimeout(() => map.resize(), 400);
}

function mapStyleCandidates(style) {
  if (style === 'raster') return [rasterStyle()];
  const selected = `https://tiles.openfreemap.org/styles/${style}`;
  return [selected, 'https://tiles.openfreemap.org/styles/liberty', 'https://tiles.openfreemap.org/styles/fiord', rasterStyle()];
}
...
function rasterStyle() {
  return {
    ...
    layers: [{ id: 'osm-base', type: 'raster', source: 'osm', paint: { 'raster-saturation': -.3, 'raster-brightness-min': .15, 'raster-brightness-max': .68, 'raster-contrast': .24 } }]
  };
}

function initializeFallbackMap() {
  state.mapKind = 'fallback';
  state.map = new FallbackWorldMap({
    container: 'world-map', initialPoint: { lat: 20, lon: 0 }, initialZoom: 1,
    onSelect: point => selectPoint(point), onEvent: event => selectEvent(event)
  });
  state.mapReady = true; $('#world-map').classList.add('map-ready');
  updateMapData(); applyLayerVisibility();
}
...
function flyTo(lat, lon, zoom = 6) {
  ...
  else state.map?.flyTo?.({ lat: Number(lat), lon: Number(lon) }, { zoom: Math.max(2, Math.round(zoom / 2)), duration: 500 });
}
...
async function boot() {
  ...
  await loadPreloads();
  updateHeader(); updateLayerButtons(); updateDrawerTabs(); renderDrawer();
  await initializeMap();
  refreshLive({ quiet: true });
  setInterval(() => refreshLive({ quiet: true }), 90_000);
}
