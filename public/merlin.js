...
import { MerlinTileMap } from './map/merlin-tile-map.js';
...
async function loadPreloads() {
  const jobs = [
    fetchJson('/data/preload-markets.json', 4_000).then(value => { if (value?.results) state.markets = value; commitPreloadState(); }),
    fetchJson('/data/preload-shipping.json', 4_000).then(value => { if (value?.ports) state.shipping = value; commitPreloadState(); }),
    fetchJson('/data/preload-shipping-catalog.json', 4_000).then(value => { if (value?.geojson) state.shippingCatalog = value; commitPreloadState(); }),
    fetchJson('/data/preload-intelligence-catalog.json', 5_000).then(value => { if (value?.countries) state.intelligenceCatalog = value; commitPreloadState(); }),
    fetchJson('/data/preload-intelligence.json', 6_000).then(value => { if (value?.countries) state.intelligence = value; commitPreloadState(); }),
    fetchJson('/data/preload-news.json', 5_000).then(value => { if (value?.articles) state.news = value; commitPreloadState(); }),
    fetchJson('/data/fallback-events.json', 7_000).then(value => { if (value?.events) state.events = value.events; commitPreloadState(); })
  ];
...
async function boot() {
  ...
  await initializeMap();
  loadPreloads().then(() => {
    setTimeout(() => refreshLive({ quiet: true }), 1_200);
  });
  setInterval(() => refreshLive({ quiet: true }), 120_000);
}
