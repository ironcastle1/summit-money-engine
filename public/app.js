...
const cache={health:null,dashboard:null,orders:[],inventory:[],products:[],activity:[],observations:[],evidence:[],priorities:[],aiThread:null};
...
async function loadAll(){
  const [health,dashboard,orders,inventory,products,alerts,activity,observations,evidence,priorities,history]=await Promise.all([
    api('/api/health'),api('/api/dashboard'),api('/api/orders?active=1'),api('/api/inventory'),api('/api/products'),api('/api/inventory/alerts'),api('/api/activity?limit=40'),api('/api/market/observations'),api('/api/market/evidence?limit=50'),api('/api/priorities?limit=14'),api('/api/ai/history?limit=80')
  ]);
  Object.assign(cache,{health,dashboard,orders,inventory,products,alerts,activity,observations,evidence,priorities});
  cache.aiThread=history.thread_id||cache.aiThread;
  render(history.messages||[]);
}

function render(history=[]){renderHealth();renderMetrics();renderPriorities();renderOrders();renderProduction();renderInventory();renderProducts();renderFinance();renderActivity();renderMarket();renderAI(history);populateSelects();}

function renderHealth(){
  $('#health').textContent='LOCAL';$('#health').className='status-pill good';
  const ai=cache.health?.local_ai||{};$('#ai-pill').textContent=ai.online&&ai.model_installed?'AI LOCAL':'AI OFFLINE';$('#ai-pill').className=`status-pill ${ai.online&&ai.model_installed?'good':'warn'}`;
  ...
  if(ai.online&&ai.model_installed){$('#system-banner').className='system-banner hidden';}
  else{$('#system-banner').className='system-banner';$('#system-banner').innerHTML=ai.online?`Local AI engine is running, but <strong>${esc(ai.model)}</strong> is not installed. Run <code>scripts\\install-local-ai.ps1</code>.`:`Local AI is not running. MERLIN's database, DXF tools and market collectors still work. Run <code>scripts\\install-local-ai.ps1</code> once, then <code>scripts\\start-merlin.ps1</code>.`}
  const sel=$('#ai-model');const models=ai.models||[];sel.innerHTML=models.length?models.map(m=>`<option ${(m===ai.model||m.startsWith(`${ai.model}:`))?'selected':''}>${esc(m)}</option>`).join(''):`<option>${esc(ai.model||'qwen2.5:3b')}</option>`;
  $('#ai-status').className=`inline-status ${ai.online&&ai.model_installed?'good':'warn'}`;$('#ai-status').textContent=ai.online&&ai.model_installed?`Local AI ready · ${ai.model} · no token billing`:`Local AI unavailable · ${ai.error||'model not installed'}`;
}
...
function renderMarket(){const rs=cache.health?.research||{};$('#research-status').className=`inline-status ${rs.last_status==='failed'?'bad':rs.evidence_count?'good':'warn'}`;$('#research-status').textContent=rs.last_status==='running'?'Market scan running…':rs.last_error?`Last scan error: ${rs.last_error}`:rs.evidence_count?`${rs.evidence_count} evidence items stored · ${rs.observation_count} interpreted observations · last scan ${date(rs.last_run_at)}`:'No market evidence stored yet. Press Scan now or leave MERLIN running for automatic scans.';$('#observations').innerHTML=(cache.observations||[]).length?cache.observations.slice(0,12).map(o=>`<div class="observation"><h3>${esc(o.topic)}</h3><p>${esc(o.observation)}</p>${o.why_valuable?`<p class="why"><strong>Why it may matter:</strong> ${esc(o.why_valuable)}</p>`:''}<details><summary>Evidence and unknowns</summary><div><strong>Direct evidence</strong><ul>${(o.direct_evidence||[]).map(x=>`<li>${esc(x)}</li>`).join('')||'<li>None recorded</li>'}</ul><strong>Unknowns</strong><ul>${(o.unknowns||[]).map(x=>`<li>${esc(x)}</li>`).join('')||'<li>None stated</li>'}</ul>${o.suggested_test?`<p><strong>Validation:</strong> ${esc(o.suggested_test)}</p>`:''}</div></details><div class="source-list">${(o.sources||[]).map(s=>`<a href="${esc(s.url)}" target="_blank" rel="noreferrer">${esc(s.publisher||s.title||'Source')}</a>`).join('')}</div></div>`).join(''):`<div class="empty">No interpreted market observations yet. Raw evidence will appear below even if the local model is offline.</div>`;$('#market-evidence').innerHTML=(cache.evidence||[]).length?cache.evidence.slice(0,40).map(e=>`<div class="evidence-item"><strong>${esc(e.title||e.query)}</strong><span>${esc(e.publisher||'')} · ${date(e.collected_at)}${e.observed_price!=null?` · ${money(e.observed_price,e.currency||'GBP')}`:''}</span><span>${esc(e.snippet||'')}</span>${/^https?:/.test(e.url)?`<a href="${esc(e.url)}" target="_blank" rel="noreferrer">Open source</a>`:''}</div>`).join(''):`<div class="empty">No raw market evidence yet.</div>`;}
function renderAI(history){const log=$('#chat-log');if(!history.length&&log.children.length===0)log.innerHTML='<div class="empty">MERLIN chat history is empty. The database is the permanent memory; chat is the natural-language interface to it.</div>';else if(history.length){log.innerHTML=history.filter(m=>['user','assistant'].includes(m.role)).map(m=>`<div class="chat ${m.role==='user'?'user':'merlin'}"><strong>${m.role==='user'?'YOU':'MERLIN'}</strong>${esc(m.content)}</div>`).join('');log.scrollTop=log.scrollHeight;}}
function appendChat(role,text){const log=$('#chat-log');if(log.querySelector('.empty'))log.innerHTML='';log.insertAdjacentHTML('beforeend',`<div class="chat ${role==='user'?'user':'merlin'}"><strong>${role==='user'?'YOU':'MERLIN'}</strong>${esc(text)}</div>`);log.scrollTop=log.scrollHeight;}
...
$('#ai-model').addEventListener('change',async e=>{try{await api('/api/ai/model',{method:'PUT',headers:jsonHeaders(),body:JSON.stringify({model:e.target.value})});await refresh();}catch(err){alert(err.message);}});
$('#chat-form').addEventListener('submit',async e=>{e.preventDefault();const input=$('#chat-input'),msg=input.value.trim();if(!msg)return;appendChat('user',msg);input.value='';try{const r=await api('/api/ai/chat',{method:'POST',headers:jsonHeaders(),body:JSON.stringify({message:msg,thread_id:cache.aiThread})});cache.aiThread=r.thread_id||cache.aiThread;appendChat('assistant',r.text);await refresh();}catch(err){appendChat('assistant',`ERROR: ${err.message}`);}});
$('#market-scan').addEventListener('click',async()=>{const b=$('#market-scan');b.disabled=true;b.textContent='Starting…';try{await api('/api/market/research',{method:'POST',headers:jsonHeaders(),body:JSON.stringify({})});await refresh();b.textContent='Scan running';setTimeout(()=>{b.disabled=false;b.textContent='Scan now';refresh().catch(()=>{});},15000);}catch(err){b.disabled=false;b.textContent='Scan now';alert(err.message);}});
