...
function createApiClient(options = {}) {
  ...
  async function get(path, params = {}, requestOptions = {}) {
    ...
    try {
      const response = await fetch(url, { headers: { accept: 'application/json', 'x-client-version': '16.9.4-instant' }, credentials: 'same-origin', signal: requestOptions.signal || controller.signal, cache: 'no-store' });
  ...
  async function post(path, body, requestOptions = {}) {
    ...
    try {
      const response = await fetch(url, {
        method: 'POST',
        headers: { accept: 'application/json', 'content-type': 'application/json', 'x-client-version': '16.9.4-instant', ...(csrfToken ? { 'x-csrf-token': csrfToken } : {}) },
...
class FallbackWorldMap {
  ...
  #renderShell() {
    ...
    this.base.src = '/assets/world-base.svg?v=16.9.4';
    ...
    this.attribution.textContent = 'SUMMIT LOCAL VECTOR BASE';
...
const STORAGE_KEY = 'summit.experience.preferences.v4';
...
function bootState(state, detail = '') {
  ...
  globalThis.__SUMMIT_BOOT__ = { state, detail, at: new Date().toISOString(), version: '16.9.4-instant' };
}
...
async function startOptionalSystems(config) {
  ...
  // Service-worker registration is intentionally disabled in 16.9.4. The
