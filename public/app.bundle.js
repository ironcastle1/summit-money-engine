...
function createApiClient(options = {}) {
  ...
  async function get(path, params = {}, requestOptions = {}) {
    ...
    try {
      const response = await fetch(url, { headers: { accept: 'application/json', 'x-client-version': '16.9.3-resilient' }, credentials: 'same-origin', signal: requestOptions.signal || controller.signal, cache: 'no-store' });
  ...
  async function post(path, body, requestOptions = {}) {
    ...
    try {
      const response = await fetch(url, {
        method: 'POST',
        headers: { accept: 'application/json', 'content-type': 'application/json', 'x-client-version': '16.9.3-resilient', ...(csrfToken ? { 'x-csrf-token': csrfToken } : {}) },
  ...
  return Object.freeze({
    ...
    marketCatalog: params => resilientMarketCatalog(params),
    ...
    marketScreener: (params, requestOptions) => resilientMarketScreener(params, requestOptions),
...
const STORAGE_KEY = 'summit.experience.preferences.v3';
...
class SoundEngine {
  constructor({ mode = 'OFF', volume = 0.12, scope = globalThis } = {}) {
    this.scope = scope;
    this.mode = mode;
