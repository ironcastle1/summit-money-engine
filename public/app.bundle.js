...
function createApiClient(options = {}) {
  ...
  async function get(path, params = {}, requestOptions = {}) {
    ...
    try {
      const response = await fetch(url, { headers: { accept: 'application/json', 'x-client-version': '16.9.2-vector' }, credentials: 'same-origin', signal: requestOptions.signal || controller.signal, cache: 'no-store' });
  ...
  async function post(path, body, requestOptions = {}) {
    ...
    try {
      const response = await fetch(url, {
        method: 'POST',
        headers: { accept: 'application/json', 'content-type': 'application/json', 'x-client-version': '16.9.2-vector', ...(csrfToken ? { 'x-csrf-token': csrfToken } : {}) },
...
function bootState(state, detail = '') {
  ...
  globalThis.__SUMMIT_BOOT__ = { state, detail, at: new Date().toISOString(), version: '16.9.2-vector' };
}
