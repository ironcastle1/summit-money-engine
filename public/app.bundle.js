/* SUMMIT MONEY MAP CLIENT BUNDLE — generated; edit source modules, not this file. */
...
// MODULE: map/geojson.js
__modules['map/geojson.js'] = (() => {
...
function circleFeature(point, radiusKm, steps = 96) {
  const coordinates = [];
...
function eventCollection(events, categoryColours) {
  return {
...
return Object.freeze({circleFeature, pointFeature, eventCollection});
})();
...
// MODULE: map/maplibre-loader.js
__modules['map/maplibre-loader.js'] = (() => {
...
async function ensureMapLibre({ timeoutMs = 8_000 } = {}) {
...
return Object.freeze({ensureMapLibre});
})();
...
const { circleFeature, pointFeature, eventCollection } = __modules['map/geojson.js'];
const { applyMapTheme, CATEGORY_COLOURS } = __modules['map/theme.js'];
const { age, number, upper } = __modules['ui/format.js'];
const { escapeHtml } = __modules['ui/dom.js'];
const { ensureMapLibre } = __modules['map/maplibre-loader.js'];
...
function renderScan(scan) {
  ...
  text('#scan-age', metrics.dataAgeMinutes === null ? 'N/A' : `${number(metrics.dataAgeMinutes)}M AGE`);
  ...
  text('#metric-source-count', `${number(metrics.sourceCount)} SOURCES`);
  text('#metric-confidence', percent(metrics.confidencePct));
  text('#metric-sample', `N=${number(metrics.sampleSize)}`);
...
class ScanController {
  ...
  async scan() {
    ...
    try {
      ...
      this.applyFilters();
      window.dispatchEvent(new CustomEvent('summit:sources-updated'));
    } catch (error) {
...
const STORAGE_KEY = 'summit.news-searches.v1';
...
class NewsController {
  ...
  async load() {
    ...
    try {
      const payload = await this.api.news(this.parameters(), { signal: this.abortController.signal, timeoutMs: 30_000 });
      this.payload = payload;
      if (!payload.stories.some(story => story.id === this.selectedId)) this.selectedId = payload.stories[0]?.id || null;
      this.render();
      window.dispatchEvent(new CustomEvent('summit:news-updated', { detail: { stories: payload.stories || [] } }));
    } catch (error) {
      if (error.name === 'AbortError') return;
      text('#news-error', `${error.code || 'NEWS_ERROR'} / ${error.message}`);
      $('#news-error')?.classList.remove('hidden');
...
const { escapeHtml } = __modules['ui/dom.js'];
const { applyMapTheme } = __modules['map/theme.js'];
...
class ShippingController {
  ...
  async refresh() {
    ...
    const button = $('#shipping-refresh'); if (button) { button.disabled = true; button.textContent = '...'; }
    try {
      const filters = this.store.getState().shippingFilters;
      const payload = await this.api.shippingSnapshot({ hours: filters.hours }, { signal: this.abortController.signal, timeoutMs: 35_000 });
      this.store.setState({ shippingSnapshot: payload }, 'shipping.snapshot_loaded');
      this.map.update(payload); this.render();
      if (!this.store.getState().shippingSelection) {
        const first = payload.chokepoints?.[0] || payload.ports?.[0];
        if (first) await this.select(payload.chokepoints?.[0] ? 'chokepoints' : 'ports', first.id, false);
      }
    } catch (error) { if (error.code !== 'TIMEOUT' || !this.abortController.signal.aborted) displayError(error); }
    finally { this.loading = false; if (button) { button.disabled = false; button.textContent = 'REFRESH'; } }
  }
  ...
  async select(type, id, focus = true) {
...
class IntelligenceController {
  ...
  async refresh() {
    ...
    const button = $('#intelligence-refresh'); if (button) { button.disabled = true; button.textContent = '...'; }
    try {
      this.overview = await this.api.intelligenceOverview({ hours: this.filters.hours, region: this.filters.region, minimumRisk: 0, limit: 300, includeNews: this.filters.includeNews }, { timeoutMs: 35_000 });
      this.map.update(this.overview); this.map.setLayer(this.filters.layer); this.render();
      if (!this.selection && this.overview.countries?.length) await this.selectCountry(this.overview.countries[0].country.iso2, false);
    } catch (error) { this.showError(error); }
    finally { this.loading = false; if (button) { button.disabled = false; button.textContent = 'REFRESH'; } }
  }
  ...
  async selectCountry(id, focus = true) {
  ...
  async selectCity(id, focus = true) {
    const city = this.catalog?.cities?.find(item => item.id === id);
    if (city && focus) this.map.focus(city);
...
class ReplayController {
  ...
  bind() {
    ...
    $('#replay-export-json')?.addEventListener('click', () => exportJson('summit-replay', this.store.getState().replayResult || {}));
    $('#replay-export-csv')?.addEventListener('click', () => exportCsv('summit-replay-trades', this.store.getState().replayResult?.trades || [], [
...
const RULES_KEY = 'summit.alert-rules.v1';
const HISTORY_KEY = 'summit.alert-history.v1';
...
class AlertController {
  ...
  bind() {
    ...
    $('#alert-export')?.addEventListener('click', () => exportJson('summit-alerts', { rules: this.repository.rules(), history: this.repository.history(), exportedAt: new Date().toISOString() }));
    $('#alert-rule-list')?.addEventListener('click', event => this.handleRuleAction(event));
    window.addEventListener('summit:opportunities-updated', event => this.evaluate(event.detail?.opportunities || [], 'OPPORTUNITY'));
    window.addEventListener('summit:news-updated', event => this.evaluate(event.detail?.stories || [], 'NEWS'));
...
const STORAGE_KEY = 'summit.workspaces.v1';
const ACTIVE_KEY = 'summit.workspace.active.v1';
...
const BUCKETS = Object.freeze({
  watchlists: { key: 'summit.market.watchlist.v1', fallback: [] },
  workspaces: { key: 'summit.workspaces.v1', fallback: [] },
  alerts: { key: 'summit.alert-rules.v1', fallback: [] },
  savedSearches: { key: 'summit.news-searches.v1', fallback: [] },
  preferences: { key: 'summit.preferences.v1', fallback: {} }
});
...
class CloudSync {
  ...
  saveLocal(bucket, value) { localStorage.setItem(BUCKETS[bucket].key, JSON.stringify(value)); window.dispatchEvent(new CustomEvent('summit:cloud-data-updated', { detail: { bucket } })); }
...
class OpsController {
  ...
  renderMetrics(metrics) {
    const requestCounter = (metrics.counters || []).filter(item => item.name === 'summit_http_requests_total').reduce((sum, item) => sum + item.value, 0);
    const errorCounter = (metrics.counters || []).filter(item => item.name === 'summit_http_errors_total' && item.labels.class === '5xx').reduce((sum, item) => sum + item.value, 0);
    const slowCounter = (metrics.counters || []).filter(item => item.name === 'summit_http_slow_requests_total').reduce((sum, item) => sum + item.value, 0);
    const latency = (metrics.histograms || []).filter(item => item.name === 'summit_http_request_duration_ms');
  ...
  exportSnapshot() {
    ...
    link.download = `summit-system-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
    link.click();
...
class ClientMetrics {
  ...
  observeErrors() {
    ...
    window.addEventListener('summit:connectivity', event => this.report({ type: 'CONNECTIVITY', name: event.detail.online ? 'ONLINE' : 'OFFLINE', online: event.detail.online, effectiveType: event.detail.effectiveType, route: location.pathname, clientVersion: this.version }).catch(() => {}));
  }
...
const SOUND_MODES = Object.freeze(['OFF', 'ALERTS', 'FULL']);
...
// MODULE: experience/sound-library.js
__modules['experience/sound-library.js'] = (() => {
...
function soundAllowed(soundName, mode) {
...
return Object.freeze({SOUND_LIBRARY, soundAllowed});
})();

// MODULE: experience/sound-engine.js
__modules['experience/sound-engine.js'] = (() => {
const { SOUND_LIBRARY, soundAllowed } = __modules['experience/sound-library.js'];
...
return Object.freeze({SoundEngine});
})();
...
const { SoundEngine } = __modules['experience/sound-engine.js'];
...
function bootState(state, detail = '') {
  ...
  globalThis.__SUMMIT_BOOT__ = { state, detail, at: new Date().toISOString(), version: '16.9.1-recovery' };
}
...
async function startOptionalSystems(config) {
  ...
  await safeStage('PWA', async () => {
    pwa = new PwaController({ onState: state => {
