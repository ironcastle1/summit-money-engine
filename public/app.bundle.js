...
function createApiClient(options = {}) {
  ...
  async function get(path, params = {}, requestOptions = {}) {
    ...
    try {
      const response = await fetch(url, { headers: { accept: 'application/json', 'x-client-version': '17.2.0-merlin' }, credentials: 'same-origin', signal: requestOptions.signal || controller.signal, cache: 'no-store' });
  ...
  async function post(path, body, requestOptions = {}) {
    ...
    try {
      const response = await fetch(url, {
        method: 'POST',
        headers: { accept: 'application/json', 'content-type': 'application/json', 'x-client-version': '17.2.0-merlin', ...(csrfToken ? { 'x-csrf-token': csrfToken } : {}) },
...
function bootState(state, detail = '') {
  ...
  globalThis.__MERLIN_BOOT__ = { state, detail, at: new Date().toISOString(), version: '17.2.0-merlin' };
}
...
async function startOptionalSystems(config) {
  ...
  // Service-worker registration is intentionally disabled in 17.2.0. The
