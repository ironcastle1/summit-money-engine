...
export class MerlinTileMap {
  constructor({ container, onSelect, onEntity, initialPoint = { lat: 22, lon: 0 }, initialZoom = 2 } = {}) {
    ...
    this.layers = { alerts: true, news: true, earthquakes: true, disasters: true, conflict: true, routes: false, ports: false, countryRisk: false, heat: true };
    this.data = { alerts: [], events: [], news: [], ports: [], routes: [], places: [] };
    this.entities = new Map();
    this.drag = null;
    this.tileUrl = navigator.webdriver ? null : 'https://tile.openstreetmap.org/{z}/{x}/{y}.png';
    this.#build();
    this.resize();
    this.render();
  }
  ...
  #build() {
    ...
    this.localBase.src = '/assets/world-base.svg?v=17.2.0';
    this.localBase.alt = '';
    ...
    this.hitLayer.addEventListener('wheel', event => {
      event.preventDefault();
      const delta = event.deltaY < 0 ? 1 : -1;
      this.zoomAround(event.clientX, event.clientY, this.zoom + delta);
    }, { passive: false });

    this.hitLayer.addEventListener('dblclick', event => {
      event.preventDefault();
      this.zoomAround(event.clientX, event.clientY, this.zoom + 1);
    });

    this.hitLayer.addEventListener('pointerdown', event => {
      if (event.button !== 0) return;
      this.drag = { x: event.clientX, y: event.clientY, center: { ...this.center }, moved: false };
      this.hitLayer.setPointerCapture?.(event.pointerId);
      this.container.classList.add('is-panning');
    });

    this.hitLayer.addEventListener('pointermove', event => {
      if (!this.drag) return;
    ...
    const finish = event => {
      ...
      this.drag = null;
      this.hitLayer.releasePointerCapture?.(event.pointerId);
      this.container.classList.remove('is-panning');
      this.tileLayer.style.transform = '';
      this.render();
      if (!moved) this.onSelect?.(this.unproject(event.clientX, event.clientY));
    };
    this.hitLayer.addEventListener('pointerup', finish);
    this.hitLayer.addEventListener('pointercancel', finish);
  ...
  setTileMode(mode) {
    if (mode === 'local') this.tileUrl = null;
    else if (mode === 'terrain') this.tileUrl = 'https://tile.opentopomap.org/{z}/{x}/{y}.png';
    else this.tileUrl = 'https://tile.openstreetmap.org/{z}/{x}/{y}.png';
    this.renderTiles();
  }
  ...
  renderTiles() {
    ...
    for (let tileY = startY; tileY <= endY; tileY += 1) {
      for (let rawX = startX; rawX <= endX; rawX += 1) {
        ...
        image.src = this.tileUrl.replace('{z}', z).replace('{x}', tileX).replace('{y}', tileY);
        image.style.left = `${rawX * TILE_SIZE - left}px`;
        image.style.top = `${tileY * TILE_SIZE - top}px`;
        image.addEventListener('load', () => this.container.classList.add('has-map-tiles'), { once: true });
