...
export class MapController {
  ...
  focusEvent(event) {
    this.flyTo({ lat: event.lat, lon: event.lon }, { zoom: Math.max(6, this.map.getZoom()), duration: 650 });
  ...
  showPopupFromProperties(properties, lngLat) {
    ...
    this.popup = new maplibregl.Popup({ maxWidth: '310px', closeButton: true, offset: 8 })
      .setLngLat(lngLat)
      .setHTML(`
