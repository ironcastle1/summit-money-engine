...
The application includes bundled, timestamped event, news and market snapshots so the core product remains populated when a public upstream throttles or fails. Supported crypto assets also attempt direct browser-side CoinGecko history and calculate empirical probabilities only from returned observations. API requests are never intercepted by the service worker, and service-worker registration is disabled in V16.9.3 to prevent stale offline responses.

## Experience controls

- `Ctrl/Command + K`: command palette
- Sound defaults to OFF. The sound button cycles OFF, ALERTS and FULL
