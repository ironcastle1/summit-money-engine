...
test('operations endpoints expose health quality build and metrics', async () => {
  ...
  assert.equal(build.version, '17.1.0-merlin');
...
test('instant local datasets and vector map assets are production-served', async () => {
  const [mapResponse, newsResponse, shippingResponse, intelligenceResponse, marketsResponse, opportunitiesResponse, bundleResponse] = await Promise.all([
    fetch(`${baseUrl}/assets/world-base.svg?v=17.1.0`),
    ...
    fetch(`${baseUrl}/merlin.js?v=17.1.0`)
  ]);
  ...
  assert.match(bundle, /new window\.maplibregl\.Map/);
  assert.match(bundle, /17\.1\.0/);
});
