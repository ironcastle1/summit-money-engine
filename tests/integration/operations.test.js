...
test('operations endpoints expose health quality build and metrics', async () => {
  ...
  assert.equal(build.version, '16.9.4-instant');
...
test('prometheus metrics and PWA assets are served', async () => {
  ...
  assert.match(await metrics.text(), /summit_http_requests_total/);
  ...
  assert.match(await serviceWorker.text(), /summit-static-/);
});

test('static assets support etag validation and compression', async () => {
  const first = await fetch(`${baseUrl}/app.bundle.js`, { headers: { 'accept-encoding': 'gzip' } });
  assert.equal(first.status, 200);
  const etag = first.headers.get('etag');
  assert.ok(etag);
  const second = await fetch(`${baseUrl}/app.bundle.js`, { headers: { 'if-none-match': etag } });
...
test('instant local datasets and vector map assets are production-served', async () => {
  const [mapResponse, newsResponse, shippingResponse, intelligenceResponse, marketsResponse, opportunitiesResponse, bundleResponse] = await Promise.all([
    fetch(`${baseUrl}/assets/world-base.svg?v=16.9.4`),
    ...
    fetch(`${baseUrl}/app.bundle.js?v=16.9.4`)
  ]);
  for (const response of [mapResponse, newsResponse, shippingResponse, intelligenceResponse, marketsResponse, opportunitiesResponse, bundleResponse]) {
    assert.equal(response.status, 200);
  }
  assert.match(mapResponse.headers.get('content-type'), /svg/);
  const [news, shipping, intelligence, markets, opportunities, bundle] = await Promise.all([
    newsResponse.json(), shippingResponse.json(), intelligenceResponse.json(), marketsResponse.json(), opportunitiesResponse.json(), bundleResponse.text()
  ]);
  ...
  assert.match(bundle, /local-detailed-map/);
  assert.match(bundle, /16\.9\.4-instant/);
});
