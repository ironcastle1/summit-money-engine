...
test('health and config endpoints return structured data', async () => {
    ...
    assert.equal(body.eventCategories.includes('earthquake'), false);
...
test('static index and bundled client are served', async () => {
    ...
    assert.match(html, /merlin-v23\.css\?v=23\.0\.0/);
    assert.doesNotMatch(html, /earthquake/i);
    assert.doesNotMatch(html, /theme-select|data-theme/);
    assert.doesNotMatch(html, /data-view=\"(?:operations|security|customers|publishing|automation)\"/i);
    const app = await fetch(`${baseUrl}/merlin-v23.js`);
    assert.equal(app.status, 200);
    assert.match(app.headers.get('content-type'), /javascript/);
    const source = await app.text();
    assert.match(source, /import \{ MapEngineV20 \}/);
    assert.match(source, /loadLiveData/);
    assert.match(source, /deriveOpportunities/);
    assert.match(source, /EARTHQUAKE_RE/);
