...
test('static index and bundled client are served', async () => {
  ...
  assert.match(html, /merlin-v18\.js/);
  const app = await fetch(`${baseUrl}/merlin-v18.js`);
  assert.equal(app.status, 200);
  assert.match(app.headers.get('content-type'), /javascript/);
  const bundle = await app.text();
  assert.match(bundle, /MerlinMapEngine/);
  assert.match(bundle, /updateMapData/);
  assert.match(bundle, /buildShippingMoney/);
...
test('intelligence source endpoint keeps credential-gated feeds explicit', async () => {
  ...
  assert.ok(['STARTING', 'ONLINE', 'DEGRADED', 'OFFLINE'].includes(body.sources.reliefweb.state));
