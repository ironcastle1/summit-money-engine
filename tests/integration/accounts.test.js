...
function sessionCookie(response) {
  ...
  const match = value.match(/merlin_session=([^;]+)/);
  return match ? `merlin_session=${match[1]}` : '';
}
...
test.before(async () => {
  directory = await mkdtemp(path.join(os.tmpdir(), 'merlin-api-account-'));
...
test('registration creates a secure session and authenticated account response', async () => {
  ...
  assert.match(cookie, /^merlin_session=/);
