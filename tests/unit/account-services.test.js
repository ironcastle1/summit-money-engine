...
async function fixture() {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'summit-account-'));
  ...
  const auth = new AuthService({ accounts, sessions, audit, secret: 'test-secret-with-more-than-thirty-two-characters', cookieName: 'summit_session', sessionTtlMs: 3600000, allowRegistration: true, bootstrap: {} });
...
test('registration, login, session authentication, and logout work together', async t => {
  ...
  const request = { headers: { cookie: `summit_session=${registered.token}` } };
