...
import { BlueskySocialSource } from '../../src/news-sources/bluesky-social-source.js';
...
test('Bluesky adapter maps posts, authors, engagement and canonical post URLs', async () => {
  const http = { json: async () => ({ posts: [{ uri: 'at://did:plc:abc/app.bsky.feed.post/xyz', indexedAt: '2026-07-29T17:00:01Z', record: { text: 'Earthquake reported in Japan', createdAt: '2026-07-29T17:00:00Z' }, author: { did: 'did:plc:abc', handle: 'reporter.bsky.social', displayName: 'Reporter' }, likeCount: 7, repostCount: 3, replyCount: 2, quoteCount: 1 }] }) };
  const source = new BlueskySocialSource(options(http));
  const result = await source.search({ query: 'earthquake', limit: 25 });
  assert.equal(result.articles.length, 1);
  assert.equal(result.articles[0].sourceType, 'SOCIAL');
  assert.equal(result.articles[0].engagement.reposts, 3);
  assert.match(result.articles[0].url, /bsky\.app\/profile/);
});
