...
test('metrics registry records counters gauges histograms and prometheus text', () => {
  ...
  registry.increment('summit_requests_total', { method: 'GET' });
  registry.increment('summit_requests_total', { method: 'GET' }, 2);
  registry.setGauge('summit_active', 4);
  registry.observe('summit_latency_ms', 25, { route: '/api/test' }, [10, 50, 100]);
  registry.observe('summit_latency_ms', 75, { route: '/api/test' }, [10, 50, 100]);
  ...
  assert.match(registry.prometheus(), /summit_latency_ms_bucket/);
});
...
test('request metrics records latency and status class once', async () => {
  ...
  complete(200);
  const snapshot = registry.snapshot();
  assert.equal(snapshot.counters.find(item => item.name === 'summit_http_requests_total').value, 1);
  assert.equal(snapshot.counters.find(item => item.name === 'summit_http_errors_total').value, 1);
  assert.equal(snapshot.histograms.find(item => item.name === 'summit_http_request_duration_ms').count, 1);
});
