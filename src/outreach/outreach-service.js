...
export async function runProspectScan(db,{location,radius_km=10,category='all',country_code='gb',limit=300,enrich_limit=35}={}){
  ...
  let origin=null,geoError=null;
  try{origin=await geocodePlace(location,country_code);}catch(error){geoError=error;}
  const scanId=id('PSCAN');
  db.prepare(`INSERT INTO prospect_scans (id,location_query,country_code,latitude,longitude,radius_km,category,status) VALUES (?,?,?,?,?,?,?,'running')`).run(scanId,location,country_code,origin?.lat??null,origin?.lon??null,Number(radius_km),category);
  try{
    let rows=[],mode='map-radius';
    if(origin){try{rows=await scanBusinessesAround({lat:origin.lat,lon:origin.lon,radiusMeters:Number(radius_km)*1000,category,limit});}catch(error){geoError=error;}}
    if(!rows.length){
      mode='web-fallback';
      const categoryText={all:'business',restaurants:'restaurant cafe pub',hospitality:'hotel restaurant cafe',gyms:'gym fitness',barbers:'barber hairdresser',retail:'shop retailer',professional:'office company',automotive:'garage car repair',trades:'trade contractor'}[category]||category;
      const found=await searchWeb(`${categoryText} ${location} official website`,Math.min(80,Math.max(15,Number(limit)||50)));
      rows=found.map((r,n)=>({source:'web_search',source_external_id:r.url,business_name:String(r.title||'').replace(/\s*[|–—-].*$/,'').trim()||new URL(r.url).hostname,category:categoryText,address:null,town:location,postcode:null,country:null,lat:null,lon:null,website:r.url,email:null,phone:null,osm_tags:{search_snippet:r.snippet||'',fallback:true,rank:n+1}}));
    }
    ...
    const upd=db.prepare(`UPDATE prospects SET business_name=?,category=?,address=COALESCE(?,address),town=COALESCE(?,town),postcode=COALESCE(?,postcode),country=COALESCE(?,country),country_code=COALESCE(?,country_code),latitude=COALESCE(?,latitude),longitude=COALESCE(?,longitude),distance_km=COALESCE(?,distance_km),website=COALESCE(?,website),email=COALESCE(?,email),email_type=COALESCE(?,email_type),phone=COALESCE(?,phone),metadata_json=?,scan_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?`);
    const prospectIds=[];
    for(const r of rows){const distance=origin&&r.lat!=null&&r.lon!=null?haversineKm(origin.lat,origin.lon,r.lat,r.lon):null,emailType=classifyEmail(r.email).email_type,compliance=country_code?.toLowerCase()==='gb'?'unknown_review':'legal_review_required',meta=JSON.stringify({osm_tags:r.osm_tags||{},scan_origin:origin?.display_name||location,scan_mode:mode,warning:geoError?.message||null}),old=find.get(r.source,r.source_external_id);
      if(old){upd.run(r.business_name,r.category,r.address,r.town,r.postcode,r.country,country_code,r.lat??null,r.lon??null,distance,r.website,r.email,emailType,r.phone,meta,scanId,old.id);updated++;prospectIds.push(old.id);}else{const pid=id('PROS');ins.run(pid,r.business_name,r.category,r.address,r.town,r.postcode,r.country,country_code,r.lat??null,r.lon??null,distance,r.website,r.email,emailType,r.phone,r.source,r.source_external_id,compliance,meta,scanId);created++;prospectIds.push(pid);}}
    db.prepare(`UPDATE prospect_scans SET status='success',result_count=?,created_count=?,updated_count=?,enriched_count=0,error=?,completed_at=CURRENT_TIMESTAMP WHERE id=?`).run(rows.length,created,updated,mode==='web-fallback'?'Radius directory unavailable; used public web results for the requested location instead.':null,scanId);
    return {scan_id:scanId,origin:origin||{display_name:location},result_count:rows.length,created,updated,enriched:0,prospect_ids:prospectIds,enrich_requested:Math.max(0,Math.min(100,Number(enrich_limit)||0)),scan_mode:mode,warning:mode==='web-fallback'?'Exact radius distances were unavailable; results came from public web search for the location.':null};
