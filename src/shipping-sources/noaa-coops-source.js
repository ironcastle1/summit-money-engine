...
export class NoaaCoopsSource extends BaseShippingSource {
  ...
  async portConditions(port) {
    ...
    return this.execute('PORT_CONDITIONS', `${port.noaaStation}:${compactDate(new Date())}`, async () => {
      ...
      const common = { application: 'merlin', begin_date: compactDate(begin), end_date: compactDate(end), datum: 'MLLW', station: port.noaaStation, time_zone: 'gmt', units: 'metric', format: 'json' };
