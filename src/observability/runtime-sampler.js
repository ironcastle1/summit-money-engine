...
export class RuntimeSampler {
  ...
  sample() {
    ...
    this.last = snapshot;
    this.metrics?.setGauge('summit_process_uptime_seconds', snapshot.uptimeSeconds);
    this.metrics?.setGauge('summit_process_memory_rss_bytes', memory.rss);
    this.metrics?.setGauge('summit_process_memory_heap_used_bytes', memory.heapUsed);
    this.metrics?.setGauge('summit_event_loop_delay_p95_ms', snapshot.eventLoopMs.p95);
    this.metrics?.setGauge('summit_cpu_load_1m', snapshot.cpu.load1);
