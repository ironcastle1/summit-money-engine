...
export class CoinbaseProvider extends BaseBillingProvider {
  ...
  async createCheckout(input) {
    ...
    const response = await fetch(`${this.baseUrl}${path}`, { method: 'POST', headers: { authorization: `Bearer ${this.token('POST', path)}`, 'content-type': 'application/json', 'x-idempotency-key': input.idempotencyKey }, body: JSON.stringify({ amount: input.amount.toFixed(2), currency: this.currency, network: this.network, description: `Summit ${input.planId} access`, successRedirectUrl: input.successUrl, failRedirectUrl: input.cancelUrl, expiresAt: new Date(Date.now() + 60 * 60 * 1000).toISOString(), metadata: { userId: input.userId, planId: input.planId, billingInterval: input.interval } }) });
