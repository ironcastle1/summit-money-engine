...
export class OwnerRecordStore {
  ...
  async list(owner, filter = {}) {
    const query = String(filter.query || filter.q || '').toLowerCase();
    const items = [...this.bucket(owner).values()].filter(item => {
      if (filter.state && String(item.state) !== String(filter.state).toUpperCase()) return false;
      if (filter.publicationId && item.publicationId !== filter.publicationId) return false;
      if (filter.active !== undefined && Boolean(item.active) !== Boolean(filter.active)) return false;
  ...
  async summary(owner) {
    const items = await this.list(owner);
    return { total: items.length, active: items.filter(item => item.active !== false && !['ARCHIVED', 'WITHDRAWN'].includes(item.state)).length };
  }
}
