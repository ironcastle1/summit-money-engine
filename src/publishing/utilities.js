...
export function unique(values, maximum = 5000) {
  return [...new Set(asArray(values).map(item => typeof item === 'string' ? clean(item, 320) : item))].slice(0, maximum);
}
...
export function deepGet(input, path, fallback = undefined) {
  ...
  for (const segment of segments) {
    if (current === null || current === undefined || !Object.prototype.hasOwnProperty.call(Object(current), segment)) return fallback;
