...
from './numbers.js';
export function regionalContagion(events = []) {
  const contributions = events.map(event => Math.min(100,
  Number(event.severity || 0) * .34 + (event.crossBorder ? 16 : 0) + (event.weapons?.includes("MISSILE") ? 10 : 0)));
  ...
  return Object.freeze({
    ...
    label: 'Risk of conflict spreading beyond the theatre',
