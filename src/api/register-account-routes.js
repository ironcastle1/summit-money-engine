...
function sessionCookies(config, result) {
  ...
  return [
    ...
    serializeCookie('summit_session_state', 'authenticated', { httpOnly: false, secure, sameSite: 'Strict', maxAge: Math.floor(config.accounts.sessionTtlMs / 1000) })
  ];
}
...
export function registerAccountRoutes(router, services) {
  ...
  router.post('/api/auth/logout', async ({ request, response, context }) => {
    ...
    response.setHeader('set-cookie', [clearCookie(services.config.accounts.cookieName, { secure: services.config.isProduction || services.config.accounts.secureCookies, sameSite: 'Strict' }), clearCookie('summit_session_state', { httpOnly: false, secure: services.config.isProduction || services.config.accounts.secureCookies, sameSite: 'Strict' })]);
