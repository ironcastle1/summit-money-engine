...
import { sendJson, sendBuffer } from '../http/response.js';
...
export function registerApiRoutes(router, services) {
  ...
  router.get('/api/map/styles', async ({ response }) => {
    sendJson(response, 200, { styles: services.mapTiles.styles(), template: '/api/map/tiles/{style}/{z}/{x}/{y}.png', generatedAt: new Date().toISOString() }, { cacheControl: 'public, max-age=3600' });
  });

  router.get('/api/map/tiles/:style/:z/:x/:y', async ({ response, params }) => {
    const yText = String(params.y || '').replace(/\.png$/i, '');
    const tile = await services.mapTiles.tile({ style: String(params.style || 'streets').toLowerCase(), z: params.z, x: params.x, y: yText });
    response.setHeader('x-map-provider', tile.provider || 'unknown');
    response.setHeader('x-cache-state', tile.cache || 'MISS');
    sendBuffer(response, 200, tile.body, tile.contentType, { cacheControl: 'public, max-age=86400, stale-while-revalidate=604800' });
  });

  router.get('/api/macro', async ({ response }) => {
    sendJson(response, 200, await services.macroMarkets.snapshot(), { cacheControl: 'public, max-age=900, stale-while-revalidate=3600' });
  });
  ...
  router.get('/api/shipping/marine', async ({ response, context }) => {
    const id = boundedString(context.query.get('id'), 'id', { min: 2, max: 96 }).toLowerCase();
    const port = services.shippingCatalog.port(id);
    if (!port) { sendJson(response, 404, { error: { code: 'PORT_NOT_FOUND', message: 'Port not found' } }); return; }
    const source = services.shippingRegistry.get('noaa-ndbc');
    sendJson(response, 200, await source.marineConditions(port));
  });
