...
export class MarketSourceRegistry {
  ...
  candidates(asset, operation, timeframeId) {
    ...
    const sourceOrder = { coingecko: 0, binance: 1, 'alpha-vantage': 2 };
    ...
    return [...local, ...preferred, ...remaining];
  }
    ...
    for (const source of candidates) {
      try {
        return await Promise.race([
          execute(source),
          new Promise((_, reject) => setTimeout(() => reject(Object.assign(new Error(`${source.name} timed out`), { code: 'SOURCE_TIMEOUT' })), source.id === 'snapshot-market' ? 1_000 : 5_000))
        ]);
      } catch (error) {
