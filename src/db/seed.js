export function seedCurrentBusiness(db) {
  ...
  // V8.1 market catalogue: prioritise genuinely different products instead of repeatedly scanning known wall-art/Etsy terms.
  const marketCatalogueVersion = db.prepare("SELECT value FROM meta WHERE key='market_catalogue_version'").get()?.value;
  if (marketCatalogueVersion !== '8.1') {
    db.prepare("UPDATE market_source_config SET enabled=0 WHERE id LIKE 'MSRC-%' OR id LIKE 'GMSRC-%'").run();
    const v8Regions = [
      ['GB','United Kingdom'],['IE','Ireland'],['US','United States'],['CA','Canada'],['MX','Mexico'],['BR','Brazil'],['AR','Argentina'],
      ['AU','Australia'],['NZ','New Zealand'],['DE','Germany'],['FR','France'],['IT','Italy'],['ES','Spain'],['PT','Portugal'],['NL','Netherlands'],
      ['BE','Belgium'],['AT','Austria'],['CH','Switzerland'],['PL','Poland'],['CZ','Czech Republic'],['HU','Hungary'],['RO','Romania'],['BG','Bulgaria'],
      ['GR','Greece'],['HR','Croatia'],['RS','Serbia'],['BA','Bosnia and Herzegovina'],['SE','Sweden'],['NO','Norway'],['DK','Denmark'],['FI','Finland'],
      ['TR','Turkey'],['AE','United Arab Emirates'],['SA','Saudi Arabia'],['QA','Qatar'],['KW','Kuwait'],['BH','Bahrain'],['OM','Oman'],['JO','Jordan'],['EG','Egypt'],
      ['MA','Morocco'],['ZA','South Africa'],['IN','India'],['JP','Japan'],['KR','South Korea'],['SG','Singapore'],['MY','Malaysia'],['ID','Indonesia'],
      ['PH','Philippines'],['TH','Thailand'],['VN','Vietnam']
    ];
    const v8Families = [
      ['house-numbers','metal house numbers address plaque house name sign'],
      ['business-signs','custom metal business logo sign reception wall shop sign'],
      ['hospitality','restaurant cafe pub hotel steel sign table number menu sign'],
      ['gym-salon','gym barber salon custom metal logo wall sign'],
      ['garden-stakes','metal garden stake plant marker garden silhouette'],
      ['plant-supports','steel plant support trellis climbing plant frame'],
      ['pet-memorials','metal pet memorial grave marker garden plaque'],
      ['memorials','metal memorial plaque grave marker remembrance sign'],
      ['wedding','metal wedding sign table number place name personalised'],
      ['bookends','steel bookends decorative metal book end'],
      ['shelf-brackets','decorative steel shelf bracket laser plasma cut'],
      ['tool-storage','steel tool rack wall holder workshop organiser'],
      ['garage-signs','garage workshop personalised metal sign'],
      ['stencils','reusable metal stencil number letter logo'],
      ['templates-gauges','steel fabrication template gauge welding square'],
      ['gate-inserts','decorative metal gate insert panel house number'],
      ['vent-grilles','decorative metal vent grille register cover'],
      ['firepit-flatpack','flat pack steel fire pit decorative panels'],
      ['garden-panels','decorative garden privacy panel steel screen'],
      ['holiday-let','holiday cottage airbnb metal house sign address plaque'],
      ['office-signs','office door sign room number metal plaque'],
      ['religious-language','Arabic Turkish Russian Japanese Korean Chinese metal name number sign'],
      ['seasonal','metal Christmas Halloween Ramadan Eid garden home sign'],
      ['wall-art-new','emerging metal wall decor niche style trend']
    ];
    const insertV8 = db.prepare(`INSERT OR REPLACE INTO market_source_config (id,name,source_type,query,enabled,notes,region,country_code,category,updated_at) VALUES (?,?,?,?,1,?,?,?,?,CURRENT_TIMESTAMP)`);
    for (const [cc,region] of v8Regions) for (const [cat,phrase] of v8Families) {
      const safeCat = cat.replace(/[^a-z0-9]+/g,'-').toUpperCase();
      insertV8.run(`V8G-${cc}-${safeCat}`,`${region} — ${cat}`,'search',`${phrase} ${region}`,'V8.1 worldwide product radar: prioritise distinct, currently makeable product families.',region,cc,cat);
    }
    db.prepare("INSERT INTO meta (key,value) VALUES ('market_catalogue_version','8.1') ON CONFLICT(key) DO UPDATE SET value=excluded.value").run();
  }
