...
export class OwnerOperationsStore {
    ...
    async put(owner, record) {
        if (!record?.id)
            throw new TypeError('Operations record id is required');
        const bucket = this.bucket(owner);
        bucket.set(record.id, Object.freeze({ ...record }));
        while (bucket.size > this.maximum)
    ...
    async list(owner, filter = {}) {
        ...
        for (const field of ['tenantId', 'serviceId', 'state', 'status', 'severity', 'environment', 'type', 'queueId', 'releaseId'])
            if (filter[field])
                rows = rows.filter(item => String(item[field] || '').toUpperCase() === String(filter[field]).toUpperCase());
        if (query)
            rows = rows.filter(item => JSON.stringify(item).toLowerCase().includes(query));
        rows.sort((a, b) => String(b.updatedAt || b.createdAt || b.recordedAt || '').localeCompare(String(a.updatedAt || a.createdAt || a.recordedAt || '')));
