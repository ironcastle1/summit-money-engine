...
export class SourceRegistry {
  ...
  async #loadAll() {
    ...
    const results = await Promise.all(entries.map(source => Promise.race([
      source.load(),
      new Promise(resolve => setTimeout(() => resolve({ events: [], health: Object.freeze({ ...source.health(), state: 'OFFLINE', errorCode: 'REGISTRY_TIMEOUT', recordCount: 0 }) }), source.id === 'snapshot' ? 500 : 1_500))
    ])));
