...
export class NewsSourceRegistry {
  ...
  async search(options = {}) {
    ...
    const results = await Promise.all(selected.map(source => Promise.race([
      ...
      new Promise(resolve => setTimeout(() => resolve({ articles: [], health: Object.freeze({ ...source.health(), state: 'OFFLINE', errorCode: 'REGISTRY_TIMEOUT', recordCount: 0 }) }), source.id === 'snapshot-news' ? 500 : 1_800))
    ])));
