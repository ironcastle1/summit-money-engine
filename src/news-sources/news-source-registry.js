...
export class NewsSourceRegistry {
  ...
  async search(options = {}) {
    ...
    const results = await Promise.all(selected.map(source => Promise.race([
      source.search(options),
      new Promise(resolve => setTimeout(() => resolve({ articles: [], health: Object.freeze({ ...source.health(), state: 'OFFLINE', errorCode: 'REGISTRY_TIMEOUT', recordCount: 0 }) }), source.id === 'snapshot-news' ? 500 : 10_000))
    ])));
