...
import { resizeDxfBuffer } from '../dxf/resize.js';
...
export function registerRoutes(app,db){
  app.get('/api/health',(req,res)=>res.json({ok:true,system:'MERLIN',version:'9.5.0',domain:'cnc-business-os-deterministic',now:new Date().toISOString(),intake:'deterministic-parser',research:marketResearchStatus(db)}));
  ...
  // DXF Resize utility — creates a new physical-size DXF and never overwrites the uploaded original.
  app.post('/api/tools/resize-dxf',upload.single('file'),(req,res)=>{
    if(!req.file)return res.status(400).json({error:'DXF file required'});
    if(path.extname(req.file.originalname).toLowerCase()!=='.dxf')return res.status(400).json({error:'Only .dxf files can be resized'});
    const machine=db.prepare('SELECT * FROM machines WHERE active=1 ORDER BY created_at LIMIT 1').get();
    const result=resizeDxfBuffer({
    ...
    const stored=`${token}_${result.outputName.replace(/[^a-zA-Z0-9._-]+/g,'_')}`;
    fs.writeFileSync(path.join(dir,stored),result.buffer);
    res.status(201).json({
      token,
  ...
  app.get('/api/tools/resize-dxf/:token/download',(req,res)=>{
    const token=String(req.params.token||'').replace(/[^a-zA-Z0-9_-]/g,'');
    if(!token)return res.status(404).end();
    const dir=path.resolve(process.env.MERLIN_GENERATED_DIR||'./data/generated/resized-dxf');
    if(!fs.existsSync(dir))return res.status(404).end();
    const file=fs.readdirSync(dir).find(name=>name.startsWith(`${token}_`)&&name.toLowerCase().endsWith('.dxf'));
    if(!file)return res.status(404).end();
    const downloadName=file.slice(token.length+1);
    res.download(path.join(dir,file),downloadName);
  });
