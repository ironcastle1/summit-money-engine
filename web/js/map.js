(function(){
  ...
  const colors={war:'#ff174f',terror:'#ff8c00',crime:'#ff3b7a',crisis:'#ffffff',politics:'#b24cff',movement:'#00d8ff',money:'#ffd94a',city:'#7aa7ff',security:'#ff326a',earthquake:'#ffffff'};
  const featureSymbols={movement:'↔',money:'£',politics:'⚑',crime:'🛡',hospital:'🏥',clinic:'⚕',pharmacy:'✚',police_station:'★',fire_station:'◆',embassy_consulate:'⚐',airport_airfield:'✈',fuel_station:'⛽',border_crossing:'⇄',port_harbour:'⚓',rail_station:'▣',main_road:'═',communications:'📡',water_point:'💧',shelter:'⌂',food_supply:'▤',power_infrastructure:'⚡'};
  ...
  function renderCountries(){if(!countryGeo||!countryLayer)return;countryLayer.clearLayers();L.geoJSON(countryGeo,{style:f=>{const name=cName(f),r=cRisk(name);let fill='#00a66a',op=.010,line=.08,w=.5;if(overlays.war&&r&&r.overlayWar){fill='#760016';op=.50;line=.95;w=1.2}else if(overlays.crisis&&r&&r.crisis>0){fill='#ff8c00';op=.18;line=.45;w=.8}else if(overlays.politics&&r&&(r.politics>0||r.crime>0)){fill='#7b2cff';op=.34;line=.72;w=1.1}else if(overlays.watch&&r&&r.watch){fill='#ffd94a';op=.13;line=.35;w=.7}return{color:fill,weight:w,opacity:line,fillColor:fill,fillOpacity:op,className:'country-fill'}},onEachFeature:(f,l)=>l.on('click',e=>{L.DomEvent.stopPropagation(e);const r=cRisk(cName(f));if(overlays.politics&&r&&(r.politics>0||r.crime>0)&&Renderers.politicsOverlayCard)Renderers.politicsOverlayCard(r);else if(overlays.crisis&&r&&r.crisis>0)Renderers.crisisOverlayCard(r);else openPlace(e.latlng.lat,e.latlng.lng);})}).addTo(countryLayer)}
