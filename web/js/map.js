(function(){
  ...
  function init(){
    map=L.map('map',{preferCanvas:true,minZoom:2,maxZoom:17,zoomControl:true,worldCopyJump:true,maxBounds:[[-85,-180],[85,180]],maxBoundsViscosity:0.25}).setView([20,12],2.85);
    window.map=map;
    ...
    baseLayer.on('tileload',()=>{baseTileLoads++;});
    baseLayer.on('tileerror',()=>{console.warn('OSM tile failed');});

    // Failsafe: if the browser/network does not load OpenStreetMap tiles, switch to CARTO.
    // This prevents the blank blue map seen in v4-9/v4-10.
    setTimeout(()=>{
      if(baseTileLoads===0 && map){
    ...
    topoLayer=L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{subdomains:'abc',opacity:0.92,attribution:'&copy; OpenStreetMap contributors',keepBuffer:3,crossOrigin:true});
