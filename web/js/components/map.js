...
const HEADERS = {
  "User-Agent": "SummitMoneyEngine/1.0 contact: local-development",
...
const EVENT_KEYWORDS = {
  war: ["war", "missile", "drone", "frontline", "battle", "invasion", "shelling", "airstrike", "troops", "military", "army", "attack on forces"],
  terror: ["terror", "terrorist", "bomb", "explosion", "attack", "gunmen", "hostage", "ied", "suicide bomber"],
  crisis: ["earthquake", "flood", "storm", "wildfire", "cyclone", "hurricane", "tornado", "landslide", "volcano", "drought", "evacuation", "humanitarian", "cholera", "famine"],
  politics: ["election", "coup", "parliament", "president", "minister", "protest", "riot", "sanction", "embassy", "government", "referendum"],
...
function centroidFromGeometry(geometry) {
  ...
  if (geometry.type === "Polygon" && Array.isArray(geometry.coordinates)) {
    ...
    return averageLonLat(ring);
  }

  if (geometry.type === "MultiPolygon" && Array.isArray(geometry.coordinates)) {
    const ring = geometry.coordinates[0] && geometry.coordinates[0][0] || [];
    return averageLonLat(ring);
  }
...
function computeRiskScores({ localCrime, homicide, events, countryName, weather, politics }) {
  ...
  return {
    safety: {
      score: safetyScore,
      status: safetyScore >= 75 ? "Lower current risk" : safetyScore >= 55 ? "Mixed" : safetyScore >= 35 ? "Elevated risk" : "High risk",
...
function buildRapidMovers(markets, events) {
  return (markets || [])
    ...
    .map((m) => {
      ...
      const score = clamp(abs * 12 + relatedEvents * 12 + (Number(m.volume) > 100000000 ? 8 : 0), 0, 100);
...
async function buildState(force = false) {
  ...
  const disasterEvents = gdacsDisasters.slice(0, 50)
  ...
  CACHE.state = {
    version: "live-scraper-expanded-2026-07-27",
...
router.get("/global-risk/point", async (req, res) => {
  ...
  try {
    ...
    const result = {
      ...
      weather,
      eventsNear,
      sourceNote: "Town-level crime changes only where official local feeds exist. UK uses data.police.uk. Other countries use national indicators, governance indicators, live events and weather."
    };
...
router.get("/local-places", async (req, res) => {
  ...
  try {
    ...
    const places = (data.elements || []).map((el) => ({
      ...
      lat: el.lat,
      lng: el.lon,
      kind: el.tags && (el.tags.place || el.tags.amenity || el.tags.emergency || el.tags.aeroway || el.tags.public_transport || "place") || "place",
      tags: el.tags || {},
